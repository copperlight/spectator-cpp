...
struct single_table_state {
  ...
  auto get_gauge_with_ttl(IdPtr id, unsigned int ttl_seconds) {
...
class base_registry {
 public:
  ...
  auto GetGauge(const IdPtr& id, unsigned int ttl_seconds) {
    return state_.get_gauge_with_ttl(final_id(id), ttl_seconds);
  }
  auto GetGauge(absl::string_view name, Tags tags = {}) {
    return GetGauge(Id::of(name, std::move(tags)));
  }
  auto GetGauge(absl::string_view name, unsigned int ttl_seconds, Tags tags = {}) {
    return GetGauge(Id::of(name, std::move(tags)), ttl_seconds);
  }
...
struct stateless {
  ...
  auto get_gauge_with_ttl(IdPtr id, unsigned int ttl_seconds = 0) {
